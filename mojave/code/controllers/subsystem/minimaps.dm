 ...
 * *: add fetching of images to allow stuff
...
/datum/controller/subsystem/minimaps/Initialize(start_timeofday)
	for(var/level=1 to length(SSmapping.z_list))
		...
		var/icon/icon_gen = new('mojave/icons/hud/minimap.dmi') //480x480 blank icon template for drawing on the map
		...
		icon_gen.Crop(1, 1, icon_gen.Width(), icon_gen.Height())
...
/datum/controller/subsystem/minimaps/proc/add_marker(atom/target, hud_flags = NONE, image/blip)
	...
	if(!initialized)
		earlyadds += CALLBACK(src, .proc/add_marker, target, hud_flags, blip)
		return
	...
	if(ismovable(target))
		...
		blip.RegisterSignal(target, COMSIG_MOVABLE_MOVED, /image/.proc/minimap_on_move)
...
/datum/controller/subsystem/minimaps/proc/get_drawing_image(zlevel, minimap_flag)
	...
	blip.icon = icon('mojave/icons/hud/minimap.dmi')
...
/atom/movable/screen/minimap/Initialize(mapload, target, flags)
	...
	var/center_x = MINIMAP_PIXEL_FROM_WORLD(hud.mymob.x)
	var/center_y = MINIMAP_PIXEL_FROM_WORLD(hud.mymob.y)
	var/icon/icon_crop = new(icon)
	icon = icon_crop.Crop(max(1, center_x - 128), max(1, center_y - 128), min(icon_crop.Width(), center_x + 128), min(icon_crop.Width(), center_y + 128))
...
/atom/movable/screen/minimap_locator
	name = "You are here"
	icon = 'mojave/icons/hud/map_blips.dmi'
	icon_state = "locator"
...
/datum/action/minimap
	name = "Toggle Minimap"
	icon_icon = 'mojave/icons/hud/actions.dmi'
...
/datum/action/minimap/Trigger(trigger_flags)
	. = ..()
	if(!map)
		return
	...
	if(minimap_displayed)
		...
		locator.RegisterSignal(tracking, COMSIG_MOVABLE_MOVED, /atom/movable/screen/minimap_locator/.proc/update)
...
/datum/action/minimap/proc/override_locator(atom/movable/to_track)
	...
	if(!minimap_displayed)
		...
		if(to_track)
			RegisterSignal(to_track, COMSIG_PARENT_QDELETING, /datum/action/minimap/.proc/clear_locator_override)
		if(owner)
			RegisterSignal(new_track, COMSIG_MOVABLE_Z_CHANGED, .proc/on_owner_z_change)
	...
	if(tracking.z != new_track.z)
		on_owner_z_change(new_track, tracking.z, new_track.z)
	locator.RegisterSignal(new_track, COMSIG_MOVABLE_MOVED, /atom/movable/screen/minimap_locator/.proc/update)
...
/datum/action/minimap/proc/clear_locator_override()
	...
	if(minimap_displayed)
		...
		locator.RegisterSignal(owner, COMSIG_MOVABLE_MOVED, /atom/movable/screen/minimap_locator/.proc/update)
		locator.update(owner)
	locator_override = null

/datum/action/minimap/Grant(mob/M)
	. = ..()
	if(M)
		var/atom/movable/tracking = locator_override ? locator_override : M
		RegisterSignal(tracking, COMSIG_MOVABLE_Z_CHANGED, .proc/on_owner_z_change)
...
/datum/action/minimap/Remove(mob/M)
	var/atom/movable/tracking = locator_override ? locator_override : M
	if(minimap_displayed)
		owner.client?.screen -= map
		owner.client?.screen -= locator
...
/datum/action/minimap/bs
	minimap_flags = MINIMAP_FLAG_BS|MINIMAP_FLAG_ENEMYMOBS
	marker_flags = MINIMAP_FLAG_BS

/datum/action/minimap/observer
	minimap_flags = MINIMAP_FLAG_BS|MINIMAP_FLAG_ENEMYMOBS|MINIMAP_FLAG_LEGION|MINIMAP_FLAG_NCR|MINIMAP_FLAG_NONEFRACTION
	marker_flags = NONE
